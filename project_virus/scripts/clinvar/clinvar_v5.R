...
clinvar$DiseaseMIM = as.character(sapply(clinvar$PhenotypeIDs, function(x) paste(grep("^OMIM:[0-9]", unlist(strsplit(x, ";|,")), value = TRUE), collapse = ";")))
...
disease_names = disease_names[nchar(disease_names$DiseaseMIM) > 0 & disease_names$Category == "Disease", ]
disease_names = unique(disease_names[, list(DiseaseMIM, DiseaseName)])
...
clin = unique(clinvar[!paste(clinvar$GeneSymbol, as.character(sapply(clinvar$HGVSp, function(x) unlist(strsplit(x, "p.", fixed = TRUE))[2])), sep = "_") %in% nclinmut, list(GeneSymbol, HGVSp, pfam, DiseaseMIM)])
