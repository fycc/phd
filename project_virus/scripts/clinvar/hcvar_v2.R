...
# get GeneID from UniProtID: ftp://ftp.uniprot.org/pub/databases/uniprot/current_release/knowledgebase/idmapping/README
...
# get GeneSymbol from GeneID
entrez2symbol = fread("/projectnb/xialab2/fyc/entrez/entrez2symbol.tab", header = FALSE, colClasses = "character", na.strings = NULL)
...
humsavar[, "GeneID" := uniprotmap[match(uniprot, `UniProtKB-AC`), `GeneID (EntrezGene)`]]
oneentrez = str_count(humsavar$GeneID, ";") == 0
humsavar[oneentrez & GeneID %in% entrez2symbol$EntrezID, "GeneSymbol" := entrez2symbol[match(GeneID, EntrezID), GeneSymbol]]
humsavar[, "GeneID" := NULL]
...
hgvs[GeneID %in% entrez2symbol$EntrezID, "GeneSymbol" := entrez2symbol[match(GeneID, EntrezID), GeneSymbol]]
hgvs[, "GeneID" := NULL]
...
###############################################
hcvar = unique(rbind(clinvar[, .(GeneSymbol, MutationID, ProteinExpression, GSExpression, pfam, PhenotypeIDs)], humsavar[, .(GeneSymbol, MutationID, ProteinExpression, GSExpression, pfam, PhenotypeIDs)]))
...
setcolorder(hcvar, c("GeneSymbol", "MutationID", "ProteinExpression", "GSExpression", "pfam", "clan", "genepfam", "geneclan", "PhenotypeSource", "PhenotypeIDs"))
...
save(hcvar_full, hcvar, file = "database/clinvar/hcvar.RData")
