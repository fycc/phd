...
#########################################################################
bactaxid = readLines("~/database/taxonomy/bactaxid.txt")
virtaxid = readLines("~/database/taxonomy/virtaxid.txt")

taxnames = fread("~/database/taxonomy/names.tab", header = FALSE, colClasses = "character")
setnames(taxnames, c("taxid", "taxname"))

bacvir = taxnames[taxid %in% union(bactaxid, virtaxid)]
bacvir[, kingdom := ifelse(taxid %in% bactaxid, "bacteria", "virus")]

taxmerged = fread("~/database/taxonomy/merged.dmp", header = FALSE)
...
intact = fread("intact_quote_replaced.txt", colClasses = "character")
