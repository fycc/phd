...
ncbibac = fread("database/intact/ncbi_taxonomy_bacteria.csv", colClasses = "character")
ncbivir = fread("database/intact/ncbi_taxonomy_virus.csv", colClasses = "character")
ncbitax = data.table(rbind(ncbibac, ncbivir), kingdom = rep(c("bacteria", "virus"), c(nrow(ncbibac), nrow(ncbivir))))
setorder(ncbitax)

uniprotbac = fread("database/intact/speclist_bacteria.txt", header = FALSE, colClasses = "character")
setnames(uniprotbac, taxcols)
uniprotvir = fread("database/intact/speclist_virus.txt", header = FALSE, colClasses = "character")
...
taxmerged = fread("database/intact/taxmerged.dmp", header = FALSE)
...
save(ncbitax, bacvir, taxmerged, file = "database/intact/taxonomy.RData")
...
intact[taxidA %in% taxmerged$taxid_old, c("taxidA") := taxmerged[match(taxidA, taxid_old), taxid_new]]
intact[taxidB %in% taxmerged$taxid_old, c("taxidB") := taxmerged[match(taxidB, taxid_old), taxid_new]]
...
intact[, c("taxnameA", "taxnameB") := lapply(.SD, function(x) ifelse(x == "9606", "Homo sapiens", bacvir$taxname[match(x, bacvir$taxid)])), .SDcols = c("taxidA", "taxidB")]
intact[, c("taxcatA", "taxcatB") := lapply(.SD, function(x) ifelse(x == "9606", "host", bacvir$kingdom[match(x, bacvir$taxid)])), .SDcols = c("taxidA", "taxidB")]
setorder(intact)
...
hpidb[taxidA %in% taxmerged$taxid_old, c("taxidA") := taxmerged[match(taxidA, taxid_old), taxid_new]]
hpidb[taxidB %in% taxmerged$taxid_old, c("taxidB") := taxmerged[match(taxidB, taxid_old), taxid_new]]
...
hpidb[, c("taxnameA", "taxnameB") := lapply(.SD, function(x) ifelse(x == "9606", "Homo sapiens", bacvir$taxname[match(x, bacvir$taxid)])), .SDcols = c("taxidA", "taxidB")]
hpidb[, c("taxcatA", "taxcatB") := lapply(.SD, function(x) ifelse(x == "9606", "host", bacvir$kingdom[match(x, bacvir$taxid)])), .SDcols = c("taxidA", "taxidB")]
setorder(hpidb)
hpidb = unique(hpidb)
...
ppi = unique(rbindlist(list(intact, hpidb), use.names = TRUE))
...
writeLines(sort(unique(c(ppi$idA, ppi$idB, ppi$cidA, ppi$cidB))), "database/intact/ppi_id.txt")
