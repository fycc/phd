...
names(ppi_list) = ppi[, paste(idA, idB, sep = "_")]
...
#####################################
# map UniProtID to GeneSymbol, since UniProt IDs are too specific (same gene can have multiple UniProt IDs)
...
ppi[nchar(geneA) == 0, "geneA" := cidA]
ppi[nchar(geneB) == 0, "geneB" := cidB]
