...
oneentrez = str_count(ppi_tab$`Cross-reference (GeneID)`, ";") == 1
...
ppi_tab[nchar(`Gene names  (primary )`) == 0, `Gene names  (primary )` := `Gene names`]
...
entrez2symbol = entrez2symbol[EntrezID %in% ppi_tab$`Cross-reference (GeneID)`]
ppi_tab[( oneentrez | (nchar(`Cross-reference (GeneID)`) > 0 & nchar(`Gene names  (primary )`) == 0) ) & `Cross-reference (GeneID)` %in% entrez2symbol$EntrezID,
        `Gene names  (primary )` := entrez2symbol[match(`Cross-reference (GeneID)`, EntrezID), GeneSymbol]]
