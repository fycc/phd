...
domain_anno[PFAM_AC %in% ipr2pf[xrefdb == "PFAM" & taxon == "Human", xref], c("human_protein_count") := lapply(.SD, function(x) ipr2pf[.(x, "Human"), max(taxon_protein_count)]), by = setdiff(names(domain_anno), "PFAM_AC")]
domain_anno[PFAM_AC %in% ipr2pf[xrefdb == "PFAM" & taxon %in% c("Virus", "Viruses"), xref], c("virus_protein_count") := lapply(.SD, function(x) ipr2pf[CJ(x, c("Virus", "Viruses")), max(taxon_protein_count)]), by = setdiff(names(domain_anno), "PFAM_AC")]
setkey(domain_anno)
